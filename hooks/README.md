...
`TeamCreate` 직후 tmux 환경을 감지하여 복구 스킬 호출 명령을 `additionalContext`로 자동 주입한다. 오케스트레이터(Claude)가 환경 감지 단계를 빠뜨려 화면 분할이 누락되는 사고를 막기 위한 강제 reminder.

- 이벤트: PostToolUse (TeamCreate)
- 타임아웃: 2초
- 동작:
  - `$TMUX` set → "Skill(\"oh-my-beom:tmux-team-agent\") 호출하세요" 컨텍스트 주입
  - unset → pass (멀티플렉서 없는 환경, 복구 불필요)
