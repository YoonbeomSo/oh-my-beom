...
- **이 hook은 `plugin.json`의 name이 정확히 `oh-my-beom`일 때만 발동한다.** 다른 플러그인 프로젝트에서는 통과 (이 플러그인 자체 개발용 룰)

### doc-sync-check (PreToolUse/Bash)

`git commit` 실행 전, `agents/`, `skills/`, `hooks/`, `rules/` 디렉토리에 파일 추가·삭제(구조적 변경)가 있는데 `docs/show/harness-architecture.html`이 같은 커밋에 포함되지 않으면 차단한다. 단순 수정(M)은 통과 — 구조 변경(A/D)만 대상.

- 이벤트: PreToolUse (Bash, git commit 명령만 대상)
- 타임아웃: 3초
- 검사 로직:
  - `git diff --cached --name-only --diff-filter=AD`에서 위 4개 디렉토리 prefix 매칭
  - 결과 있음 + HTML 미스테이징 → deny
- 건너뛰기:
  - `git commit --amend`
  - HTML 파일이 프로젝트에 없는 경우
  - 환경 변수 `SKIP_DOC_SYNC=1`
- 차단 시: 변경된 파일 목록(최대 5개)을 표시
- **이 hook은 `plugin.json`의 name이 정확히 `oh-my-beom`일 때만 발동한다.** 다른 프로젝트에서는 통과 (이 플러그인 자체 개발용 룰)

### team-recovery-reminder (PostToolUse/TeamCreate)

`TeamCreate` 직후 환경(cmux/tmux)을 감지하여 적절한 복구 스킬 호출 명령을 `additionalContext`로 자동 주입한다. 오케스트레이터(Claude)가 환경 감지 단계를 빠뜨려 화면 분할이 누락되는 사고를 막기 위한 강제 reminder.

- 이벤트: PostToolUse (TeamCreate)
- 타임아웃: 2초
- 동작:
  - `$CMUX_SOCKET` set → "Skill(\"oh-my-beom:cmux-team-agent\") 호출하세요" 컨텍스트 주입
  - `$TMUX` set → "Skill(\"oh-my-beom:tmux-team-agent\") 호출하세요" 컨텍스트 주입
  - 둘 다 unset → pass (멀티플렉서 없는 환경, 복구 불필요)
- 우회: 환경 변수 `SKIP_TEAM_RECOVERY_REMINDER=1`
