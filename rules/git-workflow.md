...
4. **커밋 후 push는 자동 실행한다.** "push 할까요?" 질문 금지. 커밋이 성공하면 즉시 push.
5. **PR/MR은 사용자 요청 시에만 생성한다.** 커밋/push 후 "PR 만들까요?" 질문 금지. `/pull-request` 명시 호출 시에만.
...
- **PR은 사용자가 `/pull-request`로 요청할 때만 생성한다.** 자동 생성 금지.
- **PR 머지 금지.** `gh pr merge` 등 머지 명령은 절대 실행하지 마라.
