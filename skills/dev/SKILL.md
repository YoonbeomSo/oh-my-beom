...
- E2E 테스트 (playwright): sonnet — 브라우저 상호작용 중심
...
**B 선택 시 흐름:** web-test-qa 완료 후 coder에게 인증 우회 코드 수정 위임 → Step 2.5-4에서 coder에게 인증 코드 원복 위임.

#### 2.5-3. 테스트 계획 + 코드 생성 + 실행/디버깅 (web-test-qa)
...
- "변경된 기능에 집중하여 /test-plan → /test-generate → /test-heal 순서로 E2E 테스트를 수행하라"
