...
필요할 때만 `Skill("oh-my-beom:...")`로 개별 호출한다. 팀에 포함되지 않는다.
...
| 감지 패턴 | 1차 대응 | 2차 대응 |
|----------|---------|---------|
| SPINNING, NO_DRIFT | `Skill("oh-my-beom:hacker")` | `Skill("oh-my-beom:research")` |
| OSCILLATION | `SendMessage(to="architect")` 설계 재검토 | 사용자에게 선택 요청 |
| DIMINISHING_RETURNS | `Skill("oh-my-beom:simplifier")` | 사용자에게 방향 전환 확인 |

### 모델 라우팅 원칙

- 산출물 생성 (PRD, 설계, 코드, 리뷰): sonnet — 비용 효율 우선
- 비판적 분석 (/design-critic 스킬): opus — 추론 깊이 우선
- 정체 탈출 (hacker, simplifier): 스킬 호출
- 브라우저 UI 테스트 (web-test-qa): 스킬 호출
- 단순 검증 (Mechanical Gate 결과 판단): 오케스트레이터가 직접 수행
- 검증/시각화 (todo-verifier, plan-visualizer): 스킬 호출
...
3. `verification.autoVisualize`가 true이면 `Skill("oh-my-beom:plan-visualizer")` 호출.
...
3. `Skill("oh-my-beom:plan-visualizer")` 호출.
...
`Skill("oh-my-beom:todo-verifier")` — 다음 정보를 포함:
...
**검증 완료 시**: `Skill("oh-my-beom:plan-visualizer")` 호출.
...
2. 미검증 `[CRITERIA]` 있으면 `Skill("oh-my-beom:todo-verifier")` 호출.
...
`Skill("oh-my-beom:web-test-qa")` — 다음 정보를 포함:
...
2. `Skill("oh-my-beom:plan-visualizer")` 호출하여 최종 HTML 생성.
