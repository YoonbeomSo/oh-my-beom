...
**오케스트레이터는 직접 코드를 작성하지 않는다.** 모든 산출물(PRD, 설계서, 코드, 리뷰)은 반드시 에이전트(Agent tool 또는 SendMessage)를 통해 생성한다. 작업이 아무리 작아도 에이전트를 호출하라. "간단하니까 직접 하겠다"는 판단을 하지 마라. 오케스트레이터의 역할은 조율(orchestrate)이지 실행(execute)이 아니다.
...
## 팀 에이전트 (TeamCreate, tmux pane)

phase-setup에서 TeamCreate로 생성되어 파이프라인 전체에 걸쳐 상시 실행된다.

| Agent | 역할 | 관점 | 모델 | color |
|-------|------|------|------|-------|
| product-owner | PRD 작성 + 인수 검증 | "뭘 만들지" / "비즈니스 의도대로 됐나" | sonnet | blue |
| architect | 설계 | "어떻게 만들지" / "구조적 일관성" | sonnet | green |
| coder | 구현 + 수정 | "만든다" | inherit | yellow |
| qa-manager | 코드 리뷰 + 스펙 충족 검증 | "스펙대로 됐나" | sonnet | cyan |
| security-auditor | 정책/보안/허점 감사 | "뭘 놓쳤나" | sonnet | magenta |

팀 에이전트에게 작업을 지시할 때는 `SendMessage(to="에이전트명")`을 사용한다.

### 스킬 (에이전트가 호출)

| 스킬 | 호출 주체 | 조건 |
|------|---------|------|
| `/design-critic` | architect | 설계 규모 중형/대형일 때 |

### 유틸리티 에이전트 (on-demand)

필요할 때만 `Agent(subagent_type="...")`로 개별 호출한다. 팀에 포함되지 않는다.

| Agent | 호출 시점 |
|-------|---------|
| todo-verifier | verification.enabled일 때 각 Phase 완료 시 |
| plan-visualizer | autoVisualize일 때 각 Phase 완료 시 |
| web-test-qa | review Phase에서 프론트엔드 변경 감지 시 (계획+생성+디버깅 통합) |

### 에스컬레이션 (정체 감지 시)

| 감지 패턴 | 1차 대응 | 2차 대응 |
|----------|---------|---------|
| SPINNING, NO_DRIFT | `Agent(subagent_type="hacker")` | `Skill("oh-my-beom:research")` |
| OSCILLATION | `SendMessage(to="architect")` 설계 재검토 | 사용자에게 선택 요청 |
| DIMINISHING_RETURNS | `Agent(subagent_type="simplifier")` | 사용자에게 방향 전환 확인 |

### 모델 라우팅 원칙

- 산출물 생성 (PRD, 설계, 코드, 리뷰): sonnet — 비용 효율 우선
- 비판적 분석 (/design-critic 스킬): opus — 추론 깊이 우선
- 정체 탈출 (hacker, simplifier): sonnet — 빠른 판단 우선
- 브라우저 UI 테스트 (web-test-qa): sonnet — 브라우저 상호작용 중심
- 단순 검증 (Mechanical Gate 결과 판단): 오케스트레이터가 직접 수행 — 에이전트 불필요
- 검증/시각화 (todo-verifier, plan-visualizer): sonnet — 경량 분석 작업
...
- **web-test-qa**: PRD의 "요구사항" + "수용 기준" + diff 파일 경로 (`DIFF_FILE`) + 코드 맵 + 개발 서버 URL + 프로젝트 루트 경로 + 최대 재시도 횟수
- **todo-verifier**: 계획 파일 경로 + 코드 맵 + 프로젝트 루트 경로
- **plan-visualizer**: 계획 파일 경로

### 병렬 실행 규칙
읽기 전용 팀원(product-owner, architect, qa-manager, security-auditor)은 서로 병렬 실행이 가능하다. 병렬 실행 시:
1. 하나의 메시지에서 여러 `SendMessage()` 호출을 동시에 발행한다.
2. 모든 팀원의 응답이 완료된 후 결과를 합산한다 (Gate 로직).
3. 쓰기 팀원(coder)은 다른 쓰기 팀원과 병렬 실행하지 않는다.
4. coder와 읽기 전용 팀원의 병렬은 **읽기 팀원이 이전 Phase의 산출물(설계서 등)만 참조하는 경우** 허용한다.
...
## 0.5b 팀 생성 (TeamCreate)

tmux 환경이면 TeamCreate로 5개 팀 에이전트를 생성한다.

1. tmux 세션 여부 확인: `echo $TMUX`가 비어있으면 이 단계를 건너뛴다 (팀 없이 기존 Agent tool 방식으로 진행).
2. TeamCreate 호출:
   - 팀명: `{브랜치명}-dev`
   - 멤버: product-owner, architect, coder, qa-manager, security-auditor
   - 각 멤버의 agentType, model, color는 위 "팀 에이전트" 테이블 참조
3. Agent tool로 각 팀원 spawn (`run_in_background: true`).
4. `Skill("oh-my-beom:tmux-team-agent")` 호출하여 빈 pane 복구.
5. 모든 팀원이 활성화되면 진행.

tmux가 아닌 환경에서는 기존 `Agent(subagent_type=...)` 방식으로 폴백한다. 이후 Phase에서 `SendMessage`를 사용하는 곳은 폴백 시 `Agent(subagent_type=...)`로 대체한다.
...
`SendMessage(to="product-owner")` — prompt에 다음을 포함:
- 기능/버그 설명: ARGS[0]
...
3. `verification.autoVisualize`가 true이면 `Agent(subagent_type="plan-visualizer")` 호출.
...
`SendMessage(to="architect")` — prompt에 다음을 포함:
- 확정된 PRD (Step 0에서 로드)
- 코드 맵 (누적된 상태)
- 프로젝트 타입, 디렉토리 구조, 컨벤션
- 프로젝트 루트 경로
...
architect에게 `/design-critic` 스킬 호출을 지시한다.
`SendMessage(to="architect")` — "설계 규모가 중형/대형이므로 `/design-critic` 스킬을 호출하여 비판 검토를 수행하라. 설계 초안, PRD, 코드 맵을 입력으로 전달하라."
...
3. `Agent(subagent_type="plan-visualizer")` 호출.
...
`SendMessage(to="coder")` — prompt에 다음을 포함:
- 확정된 설계서
- 코드 맵
- 프로젝트 타입 및 구조
- 프로젝트 루트 경로
...
`SendMessage(to="qa-manager")` — prompt에 다음을 포함:
...
`Agent(subagent_type="todo-verifier")` — prompt에 다음 포함:
...
**검증 완료 시**: `Agent(subagent_type="plan-visualizer")` 호출.
...
2. 미검증 `[CRITERIA]` 있으면 `Agent(subagent_type="todo-verifier")` 호출.
...
**Step 2**: qa-manager와 security-auditor를 **병렬로** 호출한다. 동시에 `SendMessage`를 보낸다.

`SendMessage(to="qa-manager")` — qa-manager agent.
- 변경사항 diff 파일 경로 + PRD/설계서의 관련 섹션 + 코드 맵

`SendMessage(to="security-auditor")` — security-auditor 통합 감사.
- PRD 전체 + 설계서 전체 + diff 파일 경로 + 코드 맵

**Step 2.5: 브라우저 UI 테스트 (조건부)**

프론트엔드 변경이 감지된 경우에만 실행. 백엔드 전용이면 건너뛴다.

#### 2.5-0. 프론트엔드 변경 감지
1. config.json의 `uiTesting.enabled`가 `false`면 건너뛴다.
2. `DIFF_FILE`에서 변경 파일 목록 추출 → `uiTesting.frontendFilePatterns`과 매칭.
3. 매칭 없으면 `PROJECT_ROOT`에서 `uiTesting.devServerConfigFiles` 존재 여부 확인.
4. 둘 다 해당 없으면: `execution-log`에 `"ui-test: 건너뜀 (프론트엔드 변경 없음)"` 기록, Step 3으로 진행.

#### 2.5-1. 개발 서버 시작
1. `uiTesting.devServerCommand`가 있으면 해당 명령 사용. 없으면 프로젝트 런타임에서 추론: `bun dev` 또는 `npm run dev`.
2. 백그라운드 실행, PID 캡처: `DEV_SERVER_PID=$!`
3. `uiTesting.devServerUrl`(기본 `http://localhost:3000`)에 `uiTesting.devServerStartTimeout`(기본 30초) 내 응답 확인.
4. 타임아웃 시: 서버 프로세스 정리 후 경고 표시, Step 3으로 진행.

#### 2.5-2. 인증 전략 수립
테스트 계획 수립 시 **인증 요구사항을 먼저 분석**하여 테스트 중단을 방지한다.

1. 코드 맵 + diff에서 인증/로그인 관련 코드 탐색 (미들웨어, 토큰 검증, 세션 체크 등).
2. 테스트 대상 페이지가 인증을 요구하는지 판단.
3. 인증이 필요하면 사용자에게 AskUserQuestion으로 전략을 확인:
   - **A. 테스트용 토큰/계정 제공** — 사용자가 토큰 또는 로그인 정보를 제공.
   - **B. 인증 로직 임시 우회** — 테스트 동안 인증 미들웨어/가드를 임시 비활성화 → 테스트 완료 후 원복.
   - **C. 인증 불필요** — 공개 페이지만 테스트. 그대로 진행.

**B 선택 시 흐름:** web-test-qa 완료 후 coder에게 인증 코드 원복 위임.

#### 2.5-3. 브라우저 UI 테스트 실행 (web-test-qa)
`Agent(subagent_type="web-test-qa")` — prompt에 다음 포함:
- 개발 서버 URL + PRD의 "요구사항" + "수용 기준" + diff 파일 경로 + 코드 맵
- 프로젝트 루트 경로
- 인증 전략 (있으면)
- 최대 재시도: `uiTesting.maxHealerRetries` (기본 2)
- "변경된 기능에 집중하여 /test-plan → /test-generate → /test-heal 순서로 브라우저 UI 테스트를 수행하라"

web-test-qa가 3개 스킬을 순서대로 호출하여 계획 → 생성 → 실행/디버깅을 수행한다.

web-test-qa 출력: 테스트 결과 요약 (통과/수정 후 통과/fixme/미해결 실패).

#### 2.5-4. 인증 원복 + 개발 서버 종료
1. 인증 전략 B(임시 우회)였으면: coder에게 인증 코드 원복 위임 → 원복 확인.
2. `kill ${DEV_SERVER_PID}`로 서버 종료. 포트 해제 확인.

#### 2.5-5. 결과 기록
- `execution-log`에 `{ phase: review, step: ui-test, result: "N개 pass, M개 fail, K개 fixme" }` 기록.
- Trust Ledger에 `### 브라우저 UI 테스트 (review)` 섹션을 append.

**Step 3**: qa-manager + security-auditor + 브라우저 UI 테스트 결과(있으면)를 합산한다. Trust Ledger를 저장한다. 사용자에게 **요약만** 표시.
- UI 테스트 fixme → Warning (비블로킹).
- UI 테스트 미해결 실패 → Critical.

**Step 4: 결과 처리**
- QA/ZT Critical → coder로 자동 수정 (기존)
- UI 테스트 미해결 실패 (앱 코드 문제) → coder에 UI 코드 수정 위임 → 1회 재시도
...
커밋 전에 사용자에게 작업 완료 보고를 제출하고 검토를 받는다.

**보고 내용:**
- 구현 요약 (변경된 파일 목록 + 핵심 변경 사항)
- 인수 검증 결과 (ACCEPT/REJECT)
- 리뷰 결과 요약 (QA + 보안 감사 결과)
- 미해결 항목 (있으면)

**사용자에게 AskUserQuestion으로 확인:**
- "커밋을 진행할까요?"
  - **진행** → 5.3 Commit으로 진행
  - **수정 필요** → 수정 사항을 반영하여 coder에 지시 후 재보고
  - **중단** → 현재 상태 저장 후 종료
...
2. `Agent(subagent_type="plan-visualizer")` 호출하여 최종 HTML 생성.

## 5.5 다음 단계

커밋이 완료되면 파이프라인은 종료된다.

**PR은 자동으로 생성하지 않는다.** 사용자가 `/pull-request`를 직접 요청할 때만 실행한다.
**PR 머지도 절대 실행하지 않는다** — 머지는 리뷰어가 직접 수행한다.

다음 안내를 제공한다:
"커밋이 완료되었습니다. PR이 필요하면 `/pull-request`를 실행하세요."

팀 에이전트는 유지한다 (리뷰 피드백 대응용). 사용자가 명시적으로 종료를 요청하면 팀을 정리한다.
