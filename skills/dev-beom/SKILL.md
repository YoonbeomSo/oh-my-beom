---
name: dev-beom
description: "기능 개발. 에이전트 팀(planner+architect+coder)을 실행하여 설계→구현→리뷰(qa-manager)→커밋까지 수행한다."
...
```
Bash(command="rm -f .dev/web-test-required .dev/web-test-passed .dev/diff.txt .dev/design.md .dev/codemap.md .dev/jira-context.md .dev/cleanup-report.md .dev/qa-prompt.md .dev/*.pid .dev/.qa-pane-id .dev/.team-pane-id .dev/.qa-engine .dev/.qa-surface-uuid .dev/.qa-fallback-pane .dev/.team-surface-uuid")
```

`.dev/error-log.md`는 100KB 초과 시 회전(error-learner 훅 처리).
...
planner 결과를 확인한다. planner가 질문 목록을 반환하면 **`AskUserQuestion` 도구**로 사용자에게 제시한다(각 질문은 planner가 구성한 2~4개 선택지를 옵션으로, 권장안을 첫 옵션으로 "(권장)" 표기). 사용자 답변을 planner에게 전달한다. 질문이 없으면 다음 Phase로 진행한다.
...
- `Agent(subagent_type="oh-my-beom:qa-manager")` 단일 호출
- FAIL 시 QA 루프(최대 5회)
...
1. **임시 파일 정리**:
   ...
   ```
2. **tmux pane 정리** (팀 pane이 남아있는 경우):
   ```bash
...
| coder | 설계서 + 코드 맵 |
| qa-manager (QA) | diff(파일 경로) + plan 완료 기준 + 코드 맵 (페르소나/포맷은 본인 시스템 프롬프트와 references/qa-output-format.md 참조) |
