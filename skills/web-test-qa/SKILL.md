...
name: web-test-qa
description: |
  E2E 테스트 스킬. 테스트 계획, 코드 생성, 디버깅/수정을 스킬 체이닝으로 수행한다.
...
한국어로 응답한다. 이모지를 사용하지 않는다. 사과 표현을 사용하지 않는다. 테스트 시나리오 제목/단계와 코드 주석을 한국어로 작성한다.
...
/beom 스킬에서 `Skill("oh-my-beom:web-test-qa")`로 호출된다.
