...
name: merge-request
version: 1.0.0
description: GitLab 저장소에서 커밋 히스토리로부터 제목과 본문을 자동 생성하여 MR 생성
...
현재 브랜치의 커밋 히스토리를 분석하여 GitLab Merge Request를 자동 생성한다.
...
1. `which glab`로 glab CLI 존재 확인.
   - **없으면**: AskUserQuestion — "glab CLI가 설치되어 있지 않습니다. 설치할까요?"
     - 예 → `brew install glab` 실행 (`timeout: 300000`). 설치 실패 시 **즉시 종료**.
     - 아니오 → "MR 생성을 건너뜁니다." 출력 후 **즉시 종료**.

2. `glab auth status`로 인증 확인.
   - **미인증이면**: "glab 인증이 필요합니다. `glab auth login`을 실행하겠습니다." 안내 후 `glab auth login` 실행.
     - 인증 완료 → 다음 단계 진행.
     - 인증 실패/취소 → "MR 생성을 건너뜁니다." 출력 후 **즉시 종료**.

3. origin remote가 GitLab 호스트인지 확인.
   - `git remote get-url origin` 출력에 `gitlab` 문자열이 포함되어 있는지 확인.
   - GitLab이 아닌 것으로 보이면 AskUserQuestion으로 사용자 확인 — "이 저장소는 GitLab이 아닌 것 같습니다. 계속할까요?"
     - 아니오 → "GitHub 저장소라면 `/pull-request`를 사용하세요." 안내 후 **즉시 종료**.
...
## Summary
이 MR에서 무엇을 했는지 요약한다. 핵심 접근 방식과 설계 판단을
...
1. 기존 MR 확인: `glab mr view --output json 2>/dev/null` — 이미 존재하면 URL을 표시하고 선택지 제시:
   - "업데이트": push 후 기존 MR 본문을 `glab mr update`로 갱신
   - "신규 생성": push 후 `glab mr create`
   - "취소": 스킬 종료
2. 브랜치 푸시: `git push -u origin <branch-name>` (`timeout: 120000`)
   - push 실패 시: 에러 표시 후 **즉시 종료**
3. MR 생성 (HEREDOC으로 description 전달):
   ```bash
   glab mr create --target-branch <base-branch> --title "<title>" --description "$(cat <<'EOF'
   ## Background
   ...
   ...
   ```
   - `--yes` 플래그로 대화형 프롬프트 건너뛰기 (필요 시 `--remove-source-branch`, `--squash-before-merge` 등 옵션은 사용자가 직접 지정).
   - glab mr create 실패 시: "push는 완료되었으므로 수동으로 MR을 생성해주세요." 안내 후 **즉시 종료**.
4. MR URL을 사용자에게 표시
