...
1. **권한만 변경된 파일** — `git diff <base>...HEAD --raw`로 `old mode → new mode`만 있고 내용 변화 없음.
   ...
   - 조치: `git update-index --chmod=-x|+x <file>` + `git checkout -- <file>`로 원복 권유.
2. **테스트 컴파일 fix** — `*Test.java`가 `MemberRepository` 같은 기존 코드 생성자 시그니처 변경을 따라가는 mock 추가.
   - 베이스 브랜치 컴파일이 깨진 경우 우리 PR에 끌려옴.
   - 조치: 별도 hotfix MR 분리 옵션 제시. PR에 남기는 경우 description "부수 fix" 섹션에 명시.
3. **IDE 메타데이터** — `.idea/`, `.vscode/`, `*.iml` 등.
   - 조치: 자동 unstage.
4. **dependency lockfile 무관 변경** — `package-lock.json`/`yarn.lock`에서 우리 변경과 무관한 hash 갱신.
   - 사용자에게 선택지 제시 (의도된 의존성 업데이트 vs 무관 자동 갱신).

감지 결과를 표로 출력 후 AskUserQuestion — "본 PR에 포함할까요?":
- 모두 제거 (가장 깨끗) — 권장
- 일부만 제거 (선택지 제시)
...
- 이슈 키 발견 시 description 최상단에 인용 블록으로 자동 삽입 (Jira URL이 `JIRA_BASE_URL` env에 있으면 링크 형태).
...
1. ARGS[1] (target)이 지정되었거나, base와 target이 다르면:
   ...
   - target이 base보다 N commits 앞서 있으면 (N > 0):
     - AskUserQuestion — "target이 base보다 N commits 앞서 있어 MR diff에 다른 PR이 포함될 수 있습니다. target 위로 rebase할까요?"
...
ARGS에 `--verify` 또는 사용자가 명시적으로 요청한 경우:
...
3. **MR 생성** (`--target-branch`는 ARGS[1] 또는 base):
   ```bash
   glab mr create --target-branch <target> --title "<title>" --description "$(/bin/cat <<'EOF'
   ## 작업 배경
   ...
   EOF
   )"
...
2. base 대비 우리가 변경한 파일 수와 비교 — 차이가 크면(2배 이상) target≠base 정렬이 안 됐을 가능성 → 경고.
...
- **v1.1.0**:
  - **Target ≠ Base 자동 정렬 (rebase)** — diff 부풀림 방지의 핵심
  - **부수 변경 감지** — gradlew 권한, 테스트 컴파일 fix 등 자동 식별
  - **Cross-project 링크 정규화** — short ref(`!1171`) 대신 명시적 markdown URL 강제
  - **HTTP/HTTPS 자동 안내** — 사설 GitLab 호환
  - **Post-create 검증** — changes_count, description 길이 sanity check
  - **빌드/테스트 검증 옵션** (`--verify`)
- **v1.0.0**: 초기 릴리스
