...
에이전트별 필요한 정보만 전달:
- **product-owner (PRD)**: ARGS[0] + 코드 맵 + 프로젝트 정보 + JIRA_CONTEXT + LENS_REPORT + RESEARCH_REPORT
- **product-owner (인수)**: PRD 수용 기준 + diff + 코드 맵
- **architect**: PRD(있으면) + 코드 맵 + 프로젝트 컨벤션 + JIRA_CONTEXT + RESEARCH_REPORT
...
| 파일 | 시점 |
|------|------|
| `.dev/jira-context.md` | setup에서 Jira 이슈 감지 시 생성 |
